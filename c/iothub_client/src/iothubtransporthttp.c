	...
	else
	{
		...
		if (handleData->doBatchedTransfers)
		{
			...
			else
			{
				...
				{
					IoTHubClient_LL_SendComplete(iotHubClientHandle, &(deviceData->eventConfirmations), IOTHUB_CLIENT_CONFIRMATION_BATCHSTATE_FAILED); /*takes care of emptying the list too*/
					break;
				}
